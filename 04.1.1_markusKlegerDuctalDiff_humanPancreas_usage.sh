...
orgcountsDF.drop(['DonorAge', 'Celltype', 'DonorAge_Celltype'], axis=1, inplace=True)
...
# Add annotation
adata.obs['DonorAge'] = orgdataDF['DonorAge'].astype(str)
adata.obs['Celltype'] = orgdataDF['Celltype']
adata.obs['DonorAge_Celltype'] = orgdataDF['DonorAge_Celltype']

# modified column for acinar+age and ductal+ age
...
normCorrectedDF['DonorAge']          = orgdataDF['DonorAge'].astype(str)
normCorrectedDF['Celltype']          = orgdataDF['Celltype']
normCorrectedDF['DonorAge_Celltype'] = orgdataDF['DonorAge_Celltype']
...
sc.pl.umap(adata, color=['DonorAge'], palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/02_norm_{1}_clustering_DonorAge_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adata, color=['Celltype'], palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/02_norm_{1}_clustering_Celltype_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adata, color=['DonorAge_Celltype'], palette=sc.pl.palettes.godsnot_64, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/02_norm_{1}_clustering_DonorAge_Celltype_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adata, color='n_counts', palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/02_norm_{1}_clustering_ncounts_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
...
# adata.obs['Celltype'].value_counts()                                                                                                                           Out[67]: 
# alpha          993
# acinar         409
# ductal         385
# beta           344
...
# Plot marker genes
sc.pl.rank_genes_groups(adata, key='rank_genes_cellTypes', groups=['alpha','acinar','ductal','beta','unsure','delta','mesenchymal'], fontsize=12, show=False)
plt.savefig("{0}/{1}_marker_genes_ranking_cluster_cellTypes.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
...
sc.pl.umap(adata, color=['Celltype', 'KRT19', 'KRT7', 'CFTR', 'MUC1','BHLHA15', 'PTF1A', 'CPA1', 'AMY2A'], use_raw=False, color_map=mymap, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/{1}_marker_genes_ductal_acinar_UMAPs.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
for g in ['Celltype', 'KRT19', 'KRT7', 'CFTR', 'MUC1','BHLHA15', 'PTF1A', 'CPA1', 'AMY2A']:
...
for g in ['alpha','acinar','ductal','beta','unsure','delta','mesenchymal']:
  ngDF = pd.DataFrame()
  for n in ['names', 'scores', 'logfoldchanges',  'pvals', 'pvals_adj']:
    ngDF[n] = pd.DataFrame(adata.uns['rank_genes_cellTypes'][n])[g]
  # Save dataframes
  ngDF.to_csv("{0}/02_normalizedRaw_{1}_rank_genes_cellTypes_{2}.txt".format(countsDir, projName,g), sep='\t', header=True, index=False)

namesDF    = pd.DataFrame(adata.uns['rank_genes_cellTypes']['names'])
namesDF.to_csv("{0}/02_normalizedRaw_{1}_rank_genes_cellTypes_names.txt".format(countsDir, projName), sep='\t', header=True, index=False)

scoresDF   = pd.DataFrame(adata.uns['rank_genes_cellTypes']['scores'])
scoresDF.to_csv("{0}/02_normalizedRaw_{1}_rank_genes_cellTypes_scores.txt".format(countsDir, projName), sep='\t', header=True, index=False)

logfcDF    = pd.DataFrame(adata.uns['rank_genes_cellTypes']['logfoldchanges'])
logfcDF.to_csv("{0}/02_normalizedRaw_{1}_rank_genes_cellTypes_logfoldchanges.txt".format(countsDir, projName), sep='\t', header=True, index=False)

pvalsDF    = pd.DataFrame(adata.uns['rank_genes_cellTypes']['pvals'])
pvalsDF.to_csv("{0}/02_normalizedRaw_{1}_rank_genes_cellTypes_pvals.txt".format(countsDir, projName), sep='\t', header=True, index=False)
