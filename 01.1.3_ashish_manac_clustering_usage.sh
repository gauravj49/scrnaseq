...
# Import data
# Raw data
rawData     = sc.read_text('input/manac/T_cellranger_filtered_manac_counts_raw_genesymbols.txt'); rawData.var_names_make_unique(); rawData
# AnnData object with n_obs × n_vars = 6398 × 30958

# Denoised data (DCA)
dcaData     = sc.read_text('output/manac/01_preprocessed/02_denoised_T_cellranger_filtered_manac_counts_raw_genesymbols.txt'); dcaData.var_names_make_unique(); dcaData
...
plt.savefig("{0}_top_10_genes.png".format(get_file_info(output_file)[3]) , bbox_inches='tight'); plt.close('all')
sc.pl.violin(aedata, ['n_genes', 'n_counts'], jitter=0.4, multi_panel=True,show=False)
plt.savefig("{0}_violinplot_qc.png".format(get_file_info(output_file)[3]) , bbox_inches='tight'); plt.close('all')
sc.pl.scatter(aedata, x='n_counts', y='n_genes',show=False)
plt.savefig("{0}_scatterplot_ngenes_ncounts.png".format(get_file_info(output_file)[3]) , bbox_inches='tight'); plt.close('all')

sc.pl.umap(aedata, color=gl, use_raw=False,show=False)
plt.savefig("{0}_umap_GL1.png".format(get_file_info(output_file)[3]) , bbox_inches='tight'); plt.close('all')
