...
# 2.10) Save the filtered raw adata into a file
# Write the adata object to file
adatafile  = "{0}/01_raw_{1}_adata.h5ad" .format(countsDir, projName); adata.write(adatafile)
# # Read back the filtered raw adata object
# adatafile  = "{0}/01_raw_{1}_adata.h5ad" .format(countsDir, projName); rawadata  = sc.read_h5ad(adatafile)
...
adatafile  = "{0}/02_normCC_{1}_adata.h5ad" .format(countsDir, projName); adata.write(adatafile)
# # Read back the corrected adata object
# adatafile  = "{0}/02_normCC_{1}_adata.h5ad" .format(countsDir, projName); normadata  = sc.read_h5ad(adatafile)
...
adata.obs['customClusters'].loc[(adata.obs['customClusters']=='8')|(adata.obs['customClusters']=='9')]  = 'ductal'
adata.obs['customClusters'].loc[(adata.obs['customClusters']=='3')|(adata.obs['customClusters']=='10')] = 'acinar'
adata.obs['customClusters'].loc[(adata.obs['customClusters']=='0')|(adata.obs['customClusters']=='2')|(adata.obs['customClusters']=='13')] = 'alpha'
adata.obs['customClusters'].loc[(adata.obs['customClusters']=='1')|(adata.obs['customClusters']=='4')|(adata.obs['customClusters']=='5')|(adata.obs['customClusters']=='14')] = 'beta'
...
# 13    = tcell
# 15,16   = unknown
adataCustom = adata[~((adata.obs['customClusters']=='7') | (adata.obs['customClusters']=='12') |(adata.obs['customClusters']=='13') | (adata.obs['customClusters']=='15') | (adata.obs['customClusters']=='16')) ]
...
# plt.savefig("{0}/04_{1}_CustomClusters_UMAP.pdf".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
...
# plt.savefig("{0}/04_{1}_CustomClusters_UMAP_3D.pdf".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
...
# adataCustom.obs['customClusters'].value_counts()
# beta      2362
# alpha     2100
# acinar    1126
# ductal     910
...
# 6.6) Save the cellType assigned adata into a file
# Write the adata and cadata object to file
adatafile  = "{0}/03_markerGenes_{1}_adata.h5ad" .format(countsDir, projName); adata.write(adatafile)
# # Read back the corrected adata object
# adatafile  = "{0}/03_markerGenes_{1}_adata.h5ad" .format(countsDir, projName); markeradata  = sc.read_h5ad(adatafile)
# adata = markeradata.copy()
###############################################################################################################################
# 7) Analysis of Cluster 8 + Cluster 9

# Get the cluster 8+9 anndata
...
ductalSubClusterDF = adataDuctalSubCluster.to_df()   # 894
...
# adataDuctalSubCluster.obs['subClusters'].value_counts()
...
# Get a new subcluster column
catlist = adataDuctalSubCluster.obs['subClusters'].cat.categories.tolist()
...
adataDuctalSubCluster.obs['subClusters'].loc[chmlDuctalIdx]  = 'ductal_cftrHigh_muc1Low'
adataDuctalSubCluster.obs['subClusters'].loc[clmhDuctalIdx]  = 'ductal_cftrLow_muc1High'
adataDuctalSubCluster.obs['subClusters'].loc[otherDuctalIdx] = 'ductal_other'
...
sc.tl.rank_genes_groups(adataDuctalSubCluster, groupby='subClusters', groups=['ductal_cftrHigh_muc1Low','ductal_other'], key_added='rank_genes_ref_ductal_cftrLow_muc1High', n_genes=adataDuctalSubCluster.shape[1])
...
# 7.2) Save the cellType assigned adata into a file
# Write the adata and cadata object to file
adatafile  = "{0}/04_ductalSubClusters_{1}_adata.h5ad" .format(countsDir, projName); adata.write(adatafile)
# # Read back the corrected adata object
# adatafile  = "{0}/04_ductalSubClusters_{1}_adata.h5ad" .format(countsDir, projName); markeradata  = sc.read_h5ad(adatafile)
# adata = markeradata.copy()
