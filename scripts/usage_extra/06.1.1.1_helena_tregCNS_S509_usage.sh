...
# ###################### PREPROCESSING ##########################
# 1) renamed genes.tsv to features.tsv
cd /home/rad/users/gaurav/projects/seqAnalysis/scrnaseq/input/tregCNS/10Xcompatible
dos2unix */*
macs2unix */*
for d in $(ls -d *); do mv ${d}/genes.tsv.gz ${d}/features.tsv.gz; done;
cd -

# 2) Add a third column in the features.tsv with value ("Gene Expression")
#    Need to make it 3 columns as mentioned in the 
#    https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/output/matrices
cd /home/rad/users/gaurav/projects/seqAnalysis/scrnaseq/input/tregCNS/10Xcompatible
for d in $(ls -d *); 
do
  echo ${d}
  awk -F"\t" 'BEGIN { OFS = "\t" } {$3="Gene Expression"; print}' ${d}/features.tsv > ${d}/output.tsv && mv ${d}/output.tsv ${d}/features.tsv
  echo ""
done
...
# Total number of cells                 : 1432
# Number of cells after min count filter: 1432
# Number of cells after max count filter: 1432
# Number of cells after MT filter       : 1432
# Number of cells after Ribo filter     : 1402
# Number of cells after gene filter     : 1402

# Total number of genes: 55471
# Number of genes after minCellsPergene filter: 17606
...
##################################################################
# 6.1) Marker genes & cluster annotation
...
for k,v in ma_marker_genes.items():
  ...
  plt.tight_layout()
  plt.savefig("{0}/01_{1}_mouse_cellatlas_marker_genes_adultBrain_{2}_UMAPs.png".format(markerDir, bname, k) , bbox_inches='tight', dpi=100); plt.close('all')
...
marker_list_name = "mouse_cellatlas_adultBrain_V1"
# 8.2.1) Dot plots
...
# adatafile  = "{0}/04_markerGenes_{1}_adata.loom" .format(dataDir, projName); adata.write_loom(adatafile, write_obsm_varm=False)
