...
origadata.var_names_make_unique()

# Add annotation
origadata.obs['Batch']   = orgdataDF['Batch'].astype(str)
origadata.obs['Cluster'] = orgdataDF['Cluster'].astype(str)
# AnnData object with n_obs × n_vars = 8569 × 20125 
#     obs: 'Batch', 'Cluster'

# Checking the total size of the data set
origadata.shape # We have 8569 cells and 20125 genes in the dataset

# Extract the ductal subcluster from the original data
cellBarCodes = pd.read_csv("{0}/05_{1}_clean_ductal_subcluster_cellIDs.txt".format(countsDir, projName), sep="\t", header=None).values.tolist()
cl  = sum(cellBarCodes, [])
mylist = origadata.obs.index.values
CleanDuctalSubCluster = list()
for e in mylist: 
  flag = 0
  for s in cl: 
      if s in e: 
          flag = 1 
          break
  CleanDuctalSubCluster.append(flag)
origadata.obs['CleanDuctalSubCluster'] = CleanDuctalSubCluster

# Get the clean ductal sub cluster
adata = origadata[origadata.obs['CleanDuctalSubCluster']==1,:]
...
plt.savefig("{0}/05_{1}_Batch_nCounts_plot.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')

# 2.3) Thresholding decision based on counts
p3 = sns.distplot(adata.obs['n_counts'], kde=False); #plt.show()
plt.savefig("{0}/05_{1}_ncounts_histogramplot.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
p4 = sns.distplot(adata.obs['n_counts'][adata.obs['n_counts']<2000], kde=False, bins=1000); #plt.show()
plt.savefig("{0}/05_{1}_ncounts_histogramplot_lessthan_2000.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
p5 = sns.distplot(adata.obs['n_counts'][adata.obs['n_counts']>5000], kde=False, bins=1000); #plt.show()
plt.savefig("{0}/05_{1}_ncounts_histogramplot_greaterthan_5000.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')

# 2.4) Thresholding decision based on genes
p6 = sns.distplot(adata.obs['n_genes'], kde=False, bins=1000); # plt.show()
plt.savefig("{0}/05_{1}_genes_histogramplot.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
...
plt.savefig("{0}/05_raw_{1}_clean_ductal_subcluster_ncounts_PCA.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
sc.pl.umap(adata, color=['Batch'], show=False)
plt.savefig("{0}/05_raw_{1}_clean_ductal_subcluster_Batch_UMAP.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
sc.pl.umap(adata, color=['Cluster'], show=False)
plt.savefig("{0}/05_raw_{1}_clean_ductal_subcluster_Cluster_UMAP.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
...
plt.savefig("{0}/05_{1}_scrna_sizefactors_vs_ncounts.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
sc.pl.scatter(adata, 'size_factors', 'n_genes' , show=False)
plt.savefig("{0}/05_{1}_scrna_sizefactors_vs_ngenes.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
sns.distplot(size_factors, bins=50, kde=False)
plt.savefig("{0}/05_{1}_scrna_sizefactors_histogram.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
...
plt.savefig("{0}/05_{1}_highly_variable_genes.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')
...
normCorrectedDF.to_csv("{0}/05_normalizedRaw_T_{1}_filtered.txt".format(countsDir, projName), sep='\t', header=True, index=True, index_label="PatId", float_format='%.2g')
normCorrectedDF.T.to_csv("{0}/05_normalizedRaw_{1}_filtered.txt".format(countsDir, projName), sep='\t', header=True, index=True, index_label="GeneSymbol", float_format='%.2g')

# 7) Clustering
# # 7.1) Perform clustering - using highly variable genes
for i in np.linspace(0.1,0.9,9):
  sc.tl.louvain(adata, resolution=i, key_added='louvain_r{0:0.1f}'.format(i)  , random_state=2105)
sc.tl.louvain(adata, resolution=1  , key_added='louvain_r1'  , random_state=2105)
sc.tl.louvain(adata, resolution=2  , key_added='louvain_r2'  , random_state=2105)
...
# UMAPs
sc.pl.pca_scatter(adata, color='n_counts', show=False)
plt.savefig("{0}/05_norm_{1}_ncounts_PCA.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')

sc.pl.umap(adata, color=['Batch'], palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/05_norm_{1}_Batch_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

sc.pl.umap(adata, color='n_counts', palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/05_norm_{1}_ncounts_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

sc.pl.umap(adata, color=['louvain_r0.3', 'louvain_r0.4'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9)

# UMAPs
sc.pl.umap(adata, color=['louvain_r0.3'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d')
# Rotate the plots and get he best angle ... save it interactively

sc.pl.umap(adata, color=['louvain_r0.3'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d', show=False)
plt.savefig("{0}/05_norm_{1}_louvain_r03_UMAP_3D_autoRotated.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

# Mucin and CFTR enriched ductal subpopulation
sc.pl.umap(adata, color=['MUC1', 'CFTR', 'TFF1', 'CD44'], use_raw=False, color_map=mymap, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, ncols=2, legend_loc='on data', projection='3d')
...
# sc.pl.umap(adata, color=['Cluster_ductal_sub', 'louvain_r1', 'MUC1', 'CFTR'], use_raw=False, color_map=mymap, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, legend_loc='on data')
...
# UMAPs
sc.pl.umap(adataCluster8, ecolor=['Cluster_ductal_sub'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/04_norm_{1}_louvain_r1_cluster8_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.pca_scatter(adataCluster8, color=['Cluster_ductal_sub'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/04_norm_{1}_louvain_r1_cluster8_PCA.png".format(qcDir, bname) , bbox_inches='tight'); plt.close('all')

sc.pl.umap(adataCluster8, color=['Cluster_ductal_sub'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d', show=False)
plt.savefig("{0}/04_norm_{1}_louvain_r1_cluster8_3D_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.pca_scatter(adataCluster8, color=['Cluster_ductal_sub'], palette=sc.pl.palettes.vega_10, size=100, edgecolor='k', linewidth=0.05, alpha=0.9,projection='3d', show=False)
...
adataCleanDucCluster8 = adataCluster8[~((adataCluster8.obs['Cluster_ductal_sub'] == 'schwann') | (adataCluster8.obs['Cluster_ductal_sub'] == 'acinar') | (adataCluster8.obs['Cluster_ductal_sub'] == 'activated_stellate'))]
