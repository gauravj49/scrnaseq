...
# Running Scanpy 1.4.5.1, on 2020-05-08 03:01.
# scanpy==1.4.5.1 anndata==0.7.1 umap==0.3.10 numpy==1.17.3 scipy==1.4.1 pandas==1.0.3 scikit-learn==0.21.3 stats
# models==0.10.1 python-igraph==0.7.1 louvain==0.6.1

# 1) Reading, merging and performing QC on datasets
# 1.1) Reading the data in the anndata object individually
tissueFilenames = [
                    'input/manec/tissue/bulk1079_mouse_filtered_feature_bc_matrix.h5',
...
# 1.6) Convert the sparse count matrices to dense represntation
adata.X = adata.X.toarray()
# adata.obs['tissueID'].cat.reorder_categories(['bulk1079','bulk997','bulk1001','stomach1001','bulk1018'], inplace=True)
# adata.obs['batch'].cat.reorder_categories(['4','2','3','0','1'], inplace=True)

#*******************************************************************
# 2) Quality control 
#*******************************************************************
# 2.1.1) Calculate QC covariates
...
# 2.1.2) Calculate mitochondrial/ribosomal genes fraction (percentage)
...
# 2.3) Thresholdingecision based on counts
...
# 2.5) Data quality summary plots
...
# 2.6) Filter cells according to identified QC thresholds:
...
# 2.7) Filter genes according to identified QC thresholds:
...
# Total number of cells: 26260
# Number of cells after min count filter: 26260
# Number of cells after max count filter: 26260
# Number of cells after MT filter  : 25837
# Number of cells after Ribo filter: 24098
# Number of cells after gene filter: 24098
# Total number of genes: 55488
# Number of genes after minCellsPergene filter: 28411
# Number of highly variable genes: 3999

# 2.9) Calculations for the visualizations
...
# 2D projection
ax = fig.add_subplot(2, 4, 1);                  sc.pl.umap(adata   ,                  ax=ax, color='tissueID'   , palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, show=False, title="{0} UMAP".format('tissueID'))
ax = fig.add_subplot(2, 4, 2);                  sc.pl.umap(adata   , legend_loc=None, ax=ax, color='log_counts'   , palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, show=False, title="{0} UMAP".format('log_counts'))
ax = fig.add_subplot(2, 4, 3);                  sc.pl.umap(adata   , legend_loc=None, ax=ax, color="mt_frac", palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, show=False, title="mt_frac UMAP")
ax = fig.add_subplot(2, 4, 4);                  sc.pl.umap(adata   , legend_loc=None, ax=ax, color="rb_frac"   , palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, show=False, title="rb_frac UMAP")
# 3D projection
ax = fig.add_subplot(2, 4, 5, projection='3d'); sc.pl.umap(adata   , legend_loc=None, ax=ax, color='tissueID', palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, projection='3d', show=False, title="{0} UMAP".format('tissueID'))
ax = fig.add_subplot(2, 4, 6, projection='3d'); sc.pl.umap(adata   , legend_loc=None, ax=ax, color='log_counts', palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, projection='3d', show=False, title="{0} UMAP".format('log_counts'))
ax = fig.add_subplot(2, 4, 7, projection='3d'); sc.pl.umap(adata   , legend_loc=None, ax=ax, color="mt_frac"   , palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, projection='3d', show=False, title="mt_frac UMAP")
ax = fig.add_subplot(2, 4, 8, projection='3d'); sc.pl.umap(adata   , legend_loc=None, ax=ax, color="rb_frac"   , palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, hspace=0.35, wspace=0.3, projection='3d', show=False, title="rb_frac UMAP")
plt.tight_layout()
...
sc.pp.neighbors(adata, random_state = 2105, use_rep = "mmd_latent", n_neighbors=40)
sc.tl.umap(adata, random_state = 2105, n_components=3)
...
adata.obs['cellType'].loc[adata.obs['cellType' ]=='12']  = 'Cluster12'
adata.obs['cellType'].loc[(adata.obs['cellType']=='6')|(adata.obs['cellType']=='7')] = 'Macrophages'
...
# 6,7 = Macrophages
# 8   = Fibroblasts
adataSubGroup = adata[~((adata.obs['cellType']=='Endothelial_Epithelial_Igfbp3pos') | (adata.obs['cellType']=='Erythrocytes') |(adata.obs['cellType']=='Macrophages') | (adata.obs['cellType']=='Fibroblasts'))].copy()
# adataSubGroup.shape # (1382, 8473)
...
# 1           = 'Epithelia'
# 2           = 'ECL_Enteroendocrine_PitMoucusTtf3'
# 3           = 'ProgenitorAtNeck_ChiefCellPgcHigh'
# 4           = 'Tumor4'
# 5           = 'Tumor5_Epithelia'
# 6           = 'SubCluster6'
...
sc.tl.louvain(adataSubGroup, restrict_to=('subCellType', ['C2_ECL_Enteroendocrine_PitMoucusTtf3']), resolution=0.5, key_added='subCellType_ECL_sub')
#Show the new clustering
if 'subCellType_ECL_sub_colors' in adataSubGroup.uns:
    del adataSubGroup.uns['subCellType_ECL_sub_colors']

# Subcluster keys
cluster_key            = "subCellType_ECL_sub"
cluster_bname          = "subCellType_ECL_sub"
subplot_title_fontsize = 12
subplot_title_width    = 50
...
plt.tight_layout()
plt.savefig("{0}/07_subGroup_{1}_clustering_{2}_UMAP.png".format(plotsDir, bname, cluster_bname) , bbox_inches='tight', dpi=175); plt.close('all')

# Get the new marker genes (one. vs. rest)
sc.tl.rank_genes_groups(adataSubGroup, groupby='subCellType_ECL_sub', key_added='rank_genes_subCellType_ECL_sub')
# Plot the new marker genes
sc.pl.rank_genes_groups(adataSubGroup, key='rank_genes_subCellType_ECL_sub', groups=['C2_ECL_Enteroendocrine_PitMoucusTtf3,0','C2_ECL_Enteroendocrine_PitMoucusTtf3,1','C2_ECL_Enteroendocrine_PitMoucusTtf3,2', 'C2_ECL_Enteroendocrine_PitMoucusTtf3,3'], fontsize=12, show=False)
plt.savefig("{0}/07_subGroup_{1}_clustering_{2}_marker_genes_ranking.png".format(plotsDir, bname, cluster_bname) , bbox_inches='tight', dpi=175); plt.close('all')
...
subCellTypePairComb = [comb for comb in combinations(subCellTypeCategories, 2)]
# Get pairwise plots dir
tspwsgPlotsDir = "{0}/subGroup/rankedGenes/ECLsubCluster".format(plotsDir); create_dir(tspwsgPlotsDir)
# Get pairwise data dir
tspwsgDataDir  = "{0}/subGroup/rankedGenes/ECLsubCluster".format(dataDir); create_dir(tspwsgDataDir)
# Calculate pairwise marker genes  
for grp,ref in subCellTypePairComb:
  print("- Calculating pairwise marker genes for group_v_reference: {0}_v_{1}".format(grp, ref))
  # Get genes ranking
  ...
  sc.pl.rank_genes_groups(adataSubGroup, key=keyName, groups=[grp], fontsize=12, show=False)
  # Save it in a figure
  plt.savefig("{0}/05_{1}_all_subCellType_{2}_v_{3}.png".format(tspwsgPlotsDir, bname, grp, ref) , bbox_inches='tight'); plt.close('all')
  # Get the dataframe of DE parameters
  ngDF = pd.DataFrame()
  for n in ['names', 'scores', 'logfoldchanges',  'pvals', 'pvals_adj']:
    ngDF[n] = pd.DataFrame(adataSubGroup.uns[keyName][n])[grp]
  ...
  ngDF['Reference'] = ref
  # Save the dataframe
  ngDF.to_csv("{0}/05_{1}_{2}.txt".format(tspwsgDataDir, projName, keyName), sep='\t', header=True, index=False, float_format='%.2g')
...
CellTypeDF['SubCellLouvainCluster']       = adataSubGroup.obs['louvain_r0.5']
CellTypeDF['SubCellType']                 = adataSubGroup.obs['subCellType']
CellTypeDF['SubCellTypeTumorSubClusters'] = adataSubGroup.obs['subCellType_ECL_sub']
CellTypeDF.to_csv("{0}/08_{1}_scran_normalized_counts_annotation_mit_ECL_subcluster.txt" .format(dataDir, projName, cluster_key), sep='\t', header=True, index=True, index_label="CellID")
...
# Write the adataSubGroup and cadataSubGroup object to file
adataSubGroupfile  = "{0}/08_{1}_subCellType_ECLSubCluster_adataSubGroup.h5ad" .format(dataDir, projName); adataSubGroup.write(adataSubGroupfile)
