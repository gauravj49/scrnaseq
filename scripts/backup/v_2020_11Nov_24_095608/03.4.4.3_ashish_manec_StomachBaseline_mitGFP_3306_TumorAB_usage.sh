...
adata = import_datasets(input_h5_files, sampleIdDict=sampleIdDict, batch_key=batch_key)

# 1.2) Calculate and plot QC covariates/metrices
rawadata = perform_qc(adata, plotsDir, bname, batch_key=batch_key)
...
cluster_features = [batch_key, cluster_key, 'log_counts', 'mt_frac', 'rb_frac']
...
plot_selected_cluster_umap_tsne(adata, plotsDir, bname, main_title = 'Leiden clustering', features=cluster_key, additional_features=batch_key, analysis_stage_num='03', analysis_stage='clustering', color_palette=colPalette)

# 3.65) Plot individual samples
...
# 3.76) Save the cellType assigned adata into a file
# Write the adata and cadata object to file
...
# 4.2.1) Annotation of cluster r1 with known marker genes
subplot_title_fontsize = 12
subplot_title_width    = 50
cluster_key        = "leiden_r1"
cluster_bname      = "leiden_r1"

# 4.2.2) Get all the gene names in the adata object
genespresent = adata.var.index.values.tolist()

# 4.2.3) Read the marker genes into a pandas dataframe
...
# 4.3) Save the leiden information in external file
...
# 4.5) Save the cellType assigned adata into a file
# Write the adata and cadata object to file
