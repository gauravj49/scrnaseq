...
output_dir      = "/home/rad/users/gaurav/projects/seqAnalysis/scrnaseq/output/{0}/02_scGen".format(projName); create_dir("{0}".format(output_dir))
...
adata.obs['tissueID'] = "hippocampal"
...
plt.savefig("{0}/01_{1}_cellType_nCounts_plot.png".format(plotsDir, bname) , bbox_inches='tight', dpi=175); plt.close('all')
t2 = sc.pl.violin(adata, 'mt_frac', groupby='cellType', show=False)
plt.savefig("{0}/01_{1}_cellType_mtFraction_plot.png".format(plotsDir, bname) , bbox_inches='tight', dpi=175); plt.close('all')
...
sc.pl.umap(adata, color=['cellType'], palette=sc.pl.palettes.vega_20, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d', show=False)
