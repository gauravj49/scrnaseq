...
# Analyze ductal (1077 cells) subgroups (CFTR_high/MUC1_low and CFTR_low/MUC1_high) 
# Get the index of ductal subcluster
Cluster78ductalDF = adataCluster78[adataCluster78.obs['Cluster']=='ductal'].to_df()   # 1077

# Get the ratio of expression
# pd.set_option('display.float_format', lambda x: '%.2f' % x)
Cluster78ductalDF['CFTR_rank'] = Cluster78ductalDF['CFTR'].rank()
Cluster78ductalDF['MUC1_rank'] = Cluster78ductalDF['MUC1'].rank()
Cluster78ductalDF['rank_CFTR_minus_MUC1'] = Cluster78ductalDF.apply(lambda row: (row['CFTR_rank'] - row['MUC1_rank']), axis=1)
# Cluster78ductalDF[['CFTR', 'MUC1', 'rank_CFTR_minus_MUC1']]

# Get median for both genes
# cftrMedianExp = np.float(Cluster78ductalDF['CFTR'].describe()[['50%']].values)
# muc1MedianExp = np.float(Cluster78ductalDF['MUC1'].describe()[['50%']].values)
chmlDuctalIdx  = Cluster78ductalDF[Cluster78ductalDF['rank_CFTR_minus_MUC1'] >= 100 ].index.tolist()                # 105
clmhDuctalIdx  = Cluster78ductalDF[Cluster78ductalDF['rank_CFTR_minus_MUC1'] <= -100].index.tolist()                # 94
otherDuctalIdx = Cluster78ductalDF.loc[~Cluster78ductalDF.index.isin(chmlDuctalIdx + clmhDuctalIdx)].index.tolist() # 237
...
# Get a new subcluster column
adataCluster78.obs['ductal_subcluster_Cluster78'] = adataCluster78.obs['Cluster']

# Add new categories
adataCluster78.obs['ductal_subcluster_Cluster78'].cat.add_categories(['ductal_cftrHigh_muc1Low','ductal_cftrLow_muc1High', 'ductal_other'], inplace=True) 

# Get a new subcluster column
adataCluster78.obs['ductal_subcluster_Cluster78'].loc[chmlDuctalIdx]  = adataCluster78.obs['ductal_subcluster_Cluster78'].loc[chmlDuctalIdx].replace('ductal','ductal_cftrHigh_muc1Low') 
adataCluster78.obs['ductal_subcluster_Cluster78'].loc[clmhDuctalIdx]  = adataCluster78.obs['ductal_subcluster_Cluster78'].loc[clmhDuctalIdx].replace('ductal','ductal_cftrLow_muc1High') 
adataCluster78.obs['ductal_subcluster_Cluster78'].loc[otherDuctalIdx] = adataCluster78.obs['ductal_subcluster_Cluster78'].loc[otherDuctalIdx].replace('ductal','ductal_other') 

adataCleanDucCluster78 = adataCluster78[~((adataCluster78.obs['ductal_subcluster_Cluster78'] == 'schwann') | (adataCluster78.obs['ductal_subcluster_Cluster78'] == 'acinar') | (adataCluster78.obs['ductal_subcluster_Cluster78'] == 'activated_stellate') | (adataCluster78.obs['ductal_subcluster_Cluster78'] == 'ductal_other'))]

# Visualize new subclusers
sc.pl.umap(adataCluster78, color=['ductal_subcluster_Cluster78'], palette=sc.pl.palettes.vega_10, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/04_norm_{1}_ductal_subcluster_Cluster78_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adataCluster78, color=['ductal_subcluster_Cluster78'], palette=sc.pl.palettes.vega_10, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d', show=False)
plt.savefig("{0}/04_norm_{1}_ductal_subcluster_Cluster78_UMAP_3D.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

sc.pl.umap(adataCleanDucCluster78, color=['ductal_subcluster_Cluster78'], palette=sc.pl.palettes.vega_10, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, show=False)
plt.savefig("{0}/04_norm_{1}_clean_ductal_subcluster_Cluster78_UMAP.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adataCleanDucCluster78, color=['ductal_subcluster_Cluster78'], palette=sc.pl.palettes.vega_10, size=50, edgecolor='k', linewidth=0.05, alpha=0.9, projection='3d', show=False)
plt.savefig("{0}/04_norm_{1}__clean_ductal_subcluster_Cluster78_UMAP_UMAP_3D.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

# Mucin and CFTR enriched in clean ductal subpopulation
sc.pl.umap(adataCleanDucCluster78, color=['MUC1', 'CFTR', 'TFF1', 'CD44'], use_raw=False, color_map=mymap, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, ncols=2, legend_loc='on data', show=False)
plt.savefig("{0}/04_{1}_marker_genes_mucin_cftr_enriched_clean_ductal_subpopulation_cluster78_UMAPs.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')
sc.pl.umap(adataCleanDucCluster78, color=['MUC1', 'CFTR', 'TFF1', 'CD44'], use_raw=False, color_map=mymap, size=100, edgecolor='k', linewidth=0.05, alpha=0.9, ncols=2, legend_loc='on data', projection='3d', show=False)
plt.savefig("{0}/04_{1}_marker_genes_mucin_cftr_enriched_clean_ductal_subpopulation_cluster78_3D_UMAPs.png".format(qcDir, bname) , bbox_inches='tight', dpi=300); plt.close('all')

# In [10]: Cluster78ductalDF.shape
# Out[10]: (892, 15120)
cl78DF   = Cluster78ductalDF[['CFTR', 'MUC1', 'CFTR_rank', 'MUC1_rank', 'rank_CFTR_minus_MUC1']]
cl78clDF = pd.DataFrame(); cl78clDF['Louvain_r1_clusters'] = adataCluster78.obs['louvain_r1']

# Default inner join
finalCL78DF = pd.merge(cl78DF, cl78clDF, left_index=True, right_index=True)

# Save the relevant data into an excel
finalCL78DF.to_csv("{0}/04_{1}_clean_ductal_subcluster_cluster78_gene_expression_ranks.txt".format(countsDir, projName), sep='\t', header=True, index=True, index_label="CellId", float_format='%.4g')

#########################################
# Save session
import dill
filename = "{0}/{1}_0414_ductalSubCluster.pkl".format(output_dir, projName)
